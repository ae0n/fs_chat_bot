...
class UserService[F[_]: Sync](transactor: Transactor[F])(implicit log: Logger[F]) {
  ...
  def getUserById(id: Int): F[Option[ChatUser]] = {
    ChatUsersDao.findById(id).transact(transactor).recoverWith { e =>
      log.error("Unable to get user by id", e).map(_ => None)
    }
  }

  def addUser(user: ChatUser): F[Option[ChatUser]] = {
    Sync[F].handleErrorWith(ChatUsersDao.insert(user).transact(transactor))(e => log.error("addUser error", e).map(_ => None))
  }
  ...
  def updateUserState(userId: Int, state: UserState): F[Int] = {
    ChatUsersDao.updateUserState(userId, state).transact(transactor).recoverWith { e =>
      log.error(s"Unable to update user $userId state", e).map(_ => 0)
    }
  }

  def findUsersWithState(state: UserState): F[Seq[ChatUser]] = {
    ChatUsersDao.findByStatus(state).transact(transactor).recoverWith { e =>
      log.error(s"Unable to users with state: $state", e).map(_ => Seq.empty[ChatUser])
    }
  }
